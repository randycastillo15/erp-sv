...
Reglas:
  - documentoRelacionado.tipoDocumento solo puede ser "03" (CCF) o "07" (ND).
    FE (01) NO es válida — schema enum: ["03","07"]. Lanza ValueError si inválido.
  - documento_relacionado_codigo debe ser el codigoGeneracion UUID del CCF original.
  - Receptor igual que CCF: 9 campos requeridos por schema.
...
def build_nc(request: DTEEmitRequest, numero_control: str, codigo_generacion: str) -> dict:
    ...
    if not request.documento_relacionado_codigo:
        raise ValueError(
            ...
            "Incluya el codigoGeneracion (UUID) del CCF original que se está corrigiendo."
        )

    # documentoRelacionado.tipoDocumento: schema enum ["03","07"] — FE (01) no válida
    tipo_rel = request.documento_relacionado_tipo or "03"
    if tipo_rel not in ("03", "07"):
        raise ValueError(
            f"NC: tipoDocumento relacionado '{tipo_rel}' no válido. "
            "Solo '03' (CCF) o '07' (ND) son aceptados por schema MH."
        )

    # Receptor completo — misma exigencia que CCF (9 campos requeridos por schema)
    ...
    # NC emisor omite codEstable/MH y codPuntoVenta/MH — schema no los permite
    ...
    # NC: schema exige cantidad y montos POSITIVOS (exclusiveMinimum: 0).
    # La negatividad está implícita en el tipo de documento NC.
    # tributos: ["20"] o null — NOT [] (schema minItems: 1 cuando es array).
    # numeroDocumento: string con el codigoGeneracion del CCF relacionado (≤36 chars).
    cuerpo = []
    ...
    for item in request.items:
        ...
        # NC body: no psv, no noGravado (additionalProperties: false en schema)
        cuerpo.append({
            "numItem":         item.num_item,
            "tipoItem":        item.tipo_item,
            "numeroDocumento": request.documento_relacionado_codigo,  # UUID del CCF (≤36 chars)
    ...
    # ── Resumen ───────────────────────────────────────────────────────────────
    # NC resumen: schema solo permite estos 16 campos (additionalProperties: false)
    # Omitir: porcentajeDescuento, numPagoElectronico, pagos, saldoFavor, totalNoGravado, totalPagar
    resumen = {
    ...
    # NC root: no otrosDocumentos (additionalProperties: false en schema NC)
    return {
